#!/usr/bin/env bash
#
# Try `rsphvpnudp.sh --help` for usage.
#
# (c) 2023 RSPH VPN
#
...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - RSPH VPN UDP SCRIPT server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install RSPH VPN UDP SCRIPT$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified RSPH VPN UDP SCRIPT binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove RSPH VPN UDP SCRIPT$(treset)"
	echo -e "\t$0 --remove"
	echo
...
tpl_hysteria_server_service_base() {
  ...
  cat << EOF
[Unit]
Description=RSPH VPN UDP SCRIPT Service
				...
				if [[ -z "$_is_update_required" ]]; then
					...
					if [[ -z "$_is_update_required" ]]; then
						...
						if [[ -n "$_is_frash_install" ]]; then
							echo
							echo -e "$(tbold)Congratulation! RSPH VPN UDP SCRIPT has been successfully installed on your server.$(treset)"
							echo
							echo -e "$(tbold)Client app RSPH VPN:$(treset)"
							echo -e "$(tblue)https://play.google.com/store/apps/details?id=com.reydevz.almz$(treset)"
							echo
							echo -e "ENJOY"											
							echo
							else
								...
								echo
								echo -e "$(tbold)RSPH VPN UDP SCRIPT has been successfully update to $VERSION.$(treset)"
								echo
								fi
}
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! RSPH VPN UDP SCRIPT has been successfully removed from your server.$(treset)"
	echo
...
start_services() {
	echo "Starting RSPH VPN UDP SCRIPT"
	apt update
